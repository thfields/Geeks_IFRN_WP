...
function ketos_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'ketos_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function ketos_theme() {
	return Theme::getInstance();
}

function ketos_assets() {
	return ketos_theme()->getAssetsManager();
}
...
ketos_theme()
	...
	->register_menus(
		array(
			'header-menu' => esc_html__( 'Header Menu', 'ketos' ),
			'footer-menu' => esc_html__( 'Footer Menu', 'ketos' ),
		)
	);
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	ketos_assets()
		...
		->registerStylesheet( 'ketos-theme', '/theme/theme.css' );
    ketos_assets()->loadLocalGoogleFonts();
//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
//		ketos_assets()->addGoogleFont( $family, $variants );
//	}

	add_action( 'wp_enqueue_scripts', 'ketos_print_color_scheme', 0 );
} else {
	ketos_assets()
		->registerStyle( 'ketos-theme', ketos_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function ketos_add_woocommerce_suppoert() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'ketos_add_woocommerce_suppoert' );
...
function ketos_get_builder_plugin_slug() {
...
function ketos_theme_plugins( $plugins ) {

	$plugin_slug = ketos_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'ketos' ) : __( 'Kubio', 'ketos' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'ketos_theme_plugins' );

Hooks::prefixed_add_action(
	'after_plugin_activated',
	function ( $slug ) {

		if ( $slug === ketos_get_builder_plugin_slug() ) {
				...
				<div class="kubio-admin-row get-started-imported notice notice-success">
					<div>
						<p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'ketos' ),
									ketos_theme()->getName()
								)
							);
							?>
						...
						<p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'ketos' ),
									ketos_theme()->getName()
								)
							);
							?>
					...
					<div class="button imported-view-site-button">
						<a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html_e( 'View site', 'ketos' ); ?>
						</a>
...
function ketos_try_online_url() {
    return "https://kubiobuilder.com/go/try-theme/" . get_template();
}
add_filter('kubio/editor-try-online/url', 'ketos_try_online_url', 10);


function ketos_render_header_style() {

	ketos_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'ketos_render_header_style', 100 );
