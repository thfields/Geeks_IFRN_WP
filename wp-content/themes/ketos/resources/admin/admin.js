...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".ketos-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.ketos_admin) {
    return;
  }

  var _window$ketos_admin = window.ketos_admin,
      builderStatusData = _window$ketos_admin.builderStatusData,
      getStartedData = _window$ketos_admin.getStartedData;
  ...
  function disableNotice() {
    ...
    $notice_container.closest(".ketos-admin-big-notice").find("button.notice-dismiss").click();
  }

  function toggleProcessing(value) {
    $(window).on("beforeunload.ketos-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".ketos-admin-big-notice").addClass("processing");
      $(".ketos-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".ketos-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.ketos-admin-big-notice");
    });
  }
  ...
  $notice_container.closest('.ketos-admin-big-notice').on("click", ".ketos-notice-dont-show-container", disableNotice);
  ...
  window.ketos_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
